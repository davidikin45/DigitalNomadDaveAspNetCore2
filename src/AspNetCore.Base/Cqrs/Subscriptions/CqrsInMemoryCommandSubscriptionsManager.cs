...
namespace AspNetCore.Base.DomainEvents.Subscriptions
{
    public partial class CqrsInMemoryCommandSubscriptionsManager : ICqrsCommandSubscriptionsManager
    {
        ...
        public void AddDynamicSubscription<C, R, CH>(string commandName)
           where CH : IDynamicCommandHandler<C,R>
        {
            DoAddSubscription(commandName, typeof(C), typeof(R), typeof(CH), true);
        }

        public void AddDynamicSubscription<C, CH>(string commandName)
           where CH : IDynamicCommandHandler<C>
        {
            DoAddSubscription(commandName, typeof(C), null, typeof(CH), true);
        }

        public void RemoveDynamicSubscription<C, R, CH>(string eventName)
        where CH : IDynamicCommandHandler<C,R>
        {
            var handlerToRemove = FindDynamicSubscriptionToRemove<C, R, CH>(eventName);
